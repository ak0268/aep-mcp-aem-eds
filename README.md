...
## Sidekick user impersonation

- Open Sidekick and click `Impersonate` (available in preview/live/dev).
- Enter an email; it is pushed to Alloy `identityMap` and stored in `localStorage`.
- Click `Clear` in the modal to remove the stored email and reset the identity.
- If Alloy is unavailable on the page, you will see an error notification.
