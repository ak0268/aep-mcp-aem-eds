...
export default async function decorate(block) {
  ...
  if (brandLink) {
    ...
    if (logo) {
      ...
      const isVitesse = window.location.pathname.startsWith('/vitesse') || window.location.pathname.startsWith('/content/vitesse');
      if (isAdvenzo) a.href = '/advenzo/';
      else if (isProventa) a.href = '/proventa/';
      else if (isVitesse) a.href = '/vitesse/';
      else a.href = '/';
...
export function target(block, propositions) {
  ...
  propositions.forEach((p) => {
    ...
    if (Array.isArray(p.items)) {
      p.items.forEach((item) => {
        if (
          item.data &&
          item.data.format == 'application/vnd.adobe.target.dom-action' &&
          item.data.content &&
          item.data.selector == 'head'
        ) {
          bannerElements.push(item.data.content);
          rendered = true;
        }
      });
    }
    if (rendered) {
      renderedPropositions.push(p);
    }
  });
  ...
  if (bannerElements.length > 0) {
    ...
    bannerWrapper.className = 'header-banner';
    headerEl.after(bannerWrapper);
    buildCarousel(bannerWrapper, bannerElements);
  }
